...
`actor {}` syntax. The syntax allows for various different properties to be set (see [here](reference/cricket-actorconfiguration-1.html)),
...
of the actor. It is of type `ActorCell<'a> -> Async<unit>` where the [ActorCell<'a>](reference/cricket-actorcell-1.html) 
...
(**
The `!!` operator returns an [actorSelection](reference/cricket-actorselection-0.html) type, which can represent one or many actors that 
...
will be desirable. But it is worth noting that the F# Actor Framework, provides a set of [System Messages](reference/cricket-systemmessage.html) that allow you to control
